...
- **长任务"处理完了但没回复出去"**:此前思考占位与最终答复共用一条流,15 分钟级的长任务里
  ...
  - 实测发现企微对"入站消息的迟到回复"存在约 10 分钟上下的服务端窗口(9 分钟可回、
    16.5 分钟被拒),超窗回复无法送达,详见下方"已知限制"。

### 新增

- **长连接心跳续话**:处理满 7 分钟仍未结束时,自动回一条"⏳ 仍在处理中(已 N 分钟)…"
  (此后每 2.5 分钟一条)。目的:若企微回复窗口随活动顺延则维持窗口直到完成(长任务无需
  用户干预);若窗口固定约 10 分钟,满 10 分钟后的心跳会附带"回复 #<标识> 继续"指引,
  用户回一句即开新窗口拉取结论。仅对长连接入站(reqId)生效,callback 推送不打扰。
- **落盘日志**:插件运行日志(含 SDK 发送/回执、errcode、放弃原因)同时写入
  `$DSH_HOME/logs/wecom-bot.log`(自动建目录、8MB 轮转),桌面实例 stdout 不可读时仍可复盘。

### 已知限制

- 企微智能机器人长连接对单条入站消息的回复存在服务端时效窗口(实测 9 分钟送达、
  16.5 分钟被拒,疑为 10 分钟):超过窗口后任何形式的回复(含独立新消息)都会被服务端
  拒绝,客户端重试/补发无法绕过;需缩短单轮任务或让用户用 `#<标识> 继续` 触发新一轮续接。
